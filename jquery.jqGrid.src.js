...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			_index : {},
			grouping : false,
			groupingView : {groupField:[],groupOrder:[], groupText:[],groupColumnShow:[],groupSummary:[], groupCollapse:{}, showSummaryOnHide: false, sortitems:[], sortnames:[], summary:[],summaryval:[], plusicon: 'ui-icon-circlesmall-plus', minusicon: 'ui-icon-circlesmall-minus', displayField: []},
			ignoreCase : false,
			cmTemplate : {},
			idPrefix : ""
		}, $.jgrid.defaults, pin || {});
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			if(ts.p.grouping)  {
				hiderow = ts.p.groupingView.groupCollapse.enabled === true;
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if (ts.p.grouping) {
			    var cl = ts.p.groupingView.groupCollapse.collapseLevels,
		            totalLevels = ts.p.groupingView.groupCollapse.collapseLevels.length,
                    highestLevelCollapsed;
                
			    for (highestLevelCollapsed = 0; highestLevelCollapsed<totalLevels; highestLevelCollapsed++) 
			        if (cl[highestLevelCollapsed]) 
			            break;			        
			    
                hiderow = highestLevelCollapsed < totalLevels;
				groupingPrepare = $.jgrid.getMethod("groupingPrepare");
			}
			while (i<len) {
				cur = drows[i];
...
$.jgrid.extend({
	...
	groupingToggle: function (hid) {
	    var grp = this[0].p.groupingView,
	        totalLevels = grp.groupCollapse.collapseLevels.length,
	        lastExpandedLevel = totalLevels,
	        cl = grp.groupCollapse.collapseLevels,
	        numcnt;	        
		this.each(function(){
			var $t = this,
			strpos = hid.split('_'),
			...
			if( tarspan.hasClass(minus) ) {
			    ...
			    if (r) {			        
			        for (numcnt=num+1; numcnt <= totalLevels; numcnt++) {
			            if (cl[numcnt]) {
			                lastExpandedLevel = numcnt;
			                break;
			            }
			        }
				    while (r) {				        
                        ...
                        if ($(r).hasClass('jqgroup')) {
                            var level = parseInt(r.id.split('ghead_')[1].split('_')[0]);
                            if (num < level && level <= lastExpandedLevel) {
                                $(r).show();
                                tspan = $(r).find("span." + "tree-wrap-" + $t.p.direction);
                                if (tspan && $(tspan).hasClass(plus) && (num + 1 < totalLevels && !cl[num + 1])) {
                                    $(tspan).removeClass(plus).addClass(minus);
                                }
                            }
                        } else if ($(r).hasClass('jqgrow')) {
                            if (num + 1 < lastExpandedLevel || num + 1 === totalLevels) {
                                $(r).show();
                                tspan = $(r).find("span." + "tree-wrap-" + $t.p.direction);
                                if (tspan && $(tspan).hasClass(plus) && (num + 1 < totalLevels && !cl[num + 1])) {
                                    $(tspan).removeClass(plus).addClass(minus);
				        ...
				        //if (($(r).hasClass(uid + "_" + String(num + 1)) && num < lastExpandedLevel) || ((num+1 < lastExpandedLevel || num+1 === totalLevels ) && $(r).hasClass('jqgrow'))) {
				        //    $(r).show();
				        //    tspan = $(r).find("span." + "tree-wrap-" + $t.p.direction);
				        //    if( tspan && $(tspan).hasClass(plus) && (num+1 < totalLevels && !cl[num+1]) ) {
				        //        $(tspan).removeClass(plus).addClass(minus);
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
		    ...
		    totalLevels = grp.groupCollapse.collapseLevels.length,
			highestLevelCollapsed,		   
			str = "", icon = "", hid, clid, prtl = grp.plusicon + " tree-wrap-" + $t.p.direction, mrtl = grp.minusicon + " tree-wrap-" + $t.p.direction, gv, cp=[], len =grp.groupField.length;
			for (highestLevelCollapsed = 0; highestLevelCollapsed<totalLevels-1; highestLevelCollapsed++) {
                if (cl[highestLevelCollapsed]) {
			        break;
			    }
			}
			...
			$.each(grp.groups,function(i,n){
				...
				hid = clid+"_"+i;
				icon = "<span style='cursor:pointer;' class='ui-icon " + (grp.groupCollapse.collapseLevels[n.idx] ? prtl : mrtl) + "' onclick=\"jQuery('#" + $.jgrid.jqID($t.p.id) + "').jqGrid('groupingToggle','" + hid + "');return false;\"></span>";
				try {
					gv = $t.formatter(hid, n.displayValue, cp[n.idx], n.value );
				} catch (egv) {
					gv = n.displayValue;
				}
				var hideGroup = n.idx > 0 && n.idx > highestLevelCollapsed;
				str += "<tr id=\"" + hid + "\" role=\"row\" class= \"ui-widget-content jqgroup ui-row-" + $t.p.direction + " " + clid + "\" " + (hideGroup ? "style=\"display: none;\"" : "") + "><td style=\"padding-left:" + (n.idx * 12) + "px;" + "\" colspan=\"" + colspans + "\">" + icon + $.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary) + "</td></tr>";
				...
				if( leaf ) {
					...
					for (ik = 0; ik < toEnd; ik++) {
						...
						var hhdr = "";
						if(hideGroup && !grp.showSummaryOnHide) {
